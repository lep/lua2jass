...
function _alloc takes nothing returns integer
    ...
    if _this >= JASS_MAX_ARRAY_SIZE then
        call Print#_warn("No more free instances " + __BASE_FILE__)
        return 0
    endif
...
function _free takes integer _this returns nothing
    if _this == 0 then
        call Print#_warn("Attempt to free nullptr " + __BASE_FILE__)
        return
    elseif _V[_this] != -1 then
        call Print#_warn("Attempt to double free " + __BASE_FILE__)
        return
    endif
    set _V[_this] = _F
    set _F = _this
